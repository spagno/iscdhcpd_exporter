...
DOCKER_IMAGE_NAME       ?= iscdhcpd_exporter
MACH                    ?= $(shell uname -m)
...
all:: vet checkmetrics common-all $(cross-test)
...
test-docker:
	...
	./test_image.sh "$(DOCKER_REPO)/$(DOCKER_IMAGE_NAME)-linux-amd64:$(DOCKER_IMAGE_TAG)" 9367
